...
PROGRAM track
	...
	IF (IARGC() .NE. 10) THEN
		...
		PRINT *, "nwires major_r minor_r xmax dt ntraj liptime nsep pertStrength pertIndex"
...
!	CALL setUpLoops()
	...
	CALL setUpLoopsExplicit(davesCurrents(pertIndex,2), davesCurrents(pertIndex,3),&
		davesCurrents(pertIndex,4))
	!nPert = 0
	!CALL createFinitePertWires(davesCurrents(pertIndex,1)*davesCurrents(pertIndex,2)/(2.0_8*PI))
	CALL create5PertFrames(davesCurrents(pertIndex,1)*davesCurrents(pertIndex,2)/(2.0_8*PI))
	pertStrength = davesCurrents(pertIndex,1)*davesCurrents(pertIndex,2)/(2.0_8*PI)
...
!		CALL EXIT(0)
!	END IF
!	CALL trackAndPrint(states(5302,:,:))
	!CALL calcx0Mesh()
	!CALL lowFieldSearch()
