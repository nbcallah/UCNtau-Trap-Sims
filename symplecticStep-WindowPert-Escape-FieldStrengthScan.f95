...
PROGRAM track
	...
	real(kind=PREC) :: x, y, z, px, py, pz, pert_ui
	...
	IF (.NOT. (IARGC() .EQ. 7 .OR. IARGC() .EQ. 8)) THEN
		PRINT *, "Error! Not enough or too many arguments!"
		PRINT *, "nwires major_r minor_r xmax dt numTraj pert_ui [trackToPrint]"
...
!	!PRINT *,
	...
	PRINT *, "Using no Finite Pert!!!"
	CALL createFinitePertWires(pert_ui)
	
!	PRINT *, "Finished trap setup!"
	...
	!CALL totalPotential(-0.5_8 * torus_major_r, 0.86_8 * torus_major_r, 0.0_8, xmax)
	CALL totalPotential(1.0_8, 0.0_8, 0.0_8, xmax)

	DO i=1,ntraj,1
!		CALL randomPoint(states(i,1,1), states(i,1,2), states(i,1,3),&
!			states(i,1,4), states(i,1,5), states(i,1,6), xmax)!, MASS_N*6.0_8)
		CALL randomPointBox(states(i,1,1), states(i,1,2), states(i,1,3),&
			states(i,1,4), states(i,1,5), states(i,1,6), xmax)!, MASS_N*6.0_8)
		!PRINT *, states(i,1,1)
	...
	DO i=trajPerWorker*rank+1,trajPerWorker*(rank+1),1
		!WRITE(*,"(I5,I5)") rank, i
		t=0.0_8
		IF(trackToPrint < 0) THEN
			!CALL calcLyapunov(states(i,:,:), results, t)
			!CALL rampTrack(states(i,:,:), results, t)
			CALL calcEnergy(states(i,1,:), eStart)
			PRINT *, states(i,1,:), eStart
			!WRITE(*,"(2I6,11ES28.19E3)") rank, i, results
		END IF
		IF(trackToPrint == i) THEN
